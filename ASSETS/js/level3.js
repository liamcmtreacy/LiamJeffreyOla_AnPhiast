...
function draw() {
    ...
    context.fillText(clicksLeft, badEndBox.x + badEndBox.width / 2 - 5, badEndBox.y + badEndBox.height / 2 + 5); // random guessing values fro the box
    // Dialogue
    if (mission3display) {
        ...
        context.fillText("Why do those drawings look like me, I don't like them!", 70, canvas.height - 80);
    }
...
let clickCount = 0; // separate counter
let clicksLeft = 15; // starts at 15, counts down
...
canvas.addEventListener('click', (e) => {
    ...
    if (mouseX >= badEndBox.x && mouseX <= badEndBox.x + badEndBox.width &&
        mouseY >= badEndBox.y && mouseY <= badEndBox.y + badEndBox.height) {
        clickCount++; // just adding up the clocks
        clicksLeft--;
        console.log("Box clicked:", clickCount);
